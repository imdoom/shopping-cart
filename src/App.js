...
const App = () => {
  ...
  const sizeFilter = () => {
    ...
    sizes.length > 0
      ? sizes.forEach((s) => {
          keys.forEach((key) => {
            if (inventory[key][s] > 0 && !arr.find((i) => i.sku == key))
              arr.push(products.find((p) => p.sku == key));
          });
        })
      : (arr = [...products]);
    return arr;
  ...
  useEffect(() => {
    firebase.auth().onAuthStateChanged((user) => {
      if (user) {
        ...
        setUser(user);
        setResetSize(false);
        const userCartRef = db.child("carts").child(user.uid);
        userCartRef
          .once("value")
          .then((snap) => {
            const val = snap.val();
            if (val) {
              setCartProducts(val.cartProducts || []);
              setSizes(val.sizes || []);
            }
          })
          .catch((err) => console.error("Failed to fetch saved cart:", err));
      } else {
        setUser(null);
        setCartProducts([]);
        setSizes([]);
  ...
  const SignIn = () => (
    <Button
      ...
      onClick={() =>
        firebase
          .auth()
          .signInWithPopup(new firebase.auth.GoogleAuthProvider(setUser))
      }
    >
      Sign in with Google
    </Button>
  ...
  return (
    <div id="app">
      <Column.Group>
        ...
        <Column size={2}>
          <Button.Group textAlign="left" narrow>
            <Button
              rounded
              onClick={() => sizeSelection("S")}
              color={sizes.includes("S") ? "black" : ""}
              className="sizeBtn"
            >
              S
            </Button>
            <Button
              rounded
              onClick={() => sizeSelection("M")}
              color={sizes.includes("M") ? "black" : ""}
              className="sizeBtn"
            >
              M
            </Button>
            <Button
              rounded
              onClick={() => sizeSelection("L")}
              color={sizes.includes("L") ? "black" : ""}
              className="sizeBtn"
            >
              L
            </Button>
            <Button
              rounded
              onClick={() => sizeSelection("XL")}
              color={sizes.includes("XL") ? "black" : ""}
              className="sizeBtn"
            >
              XL
            </Button>
      ...
      <ShoppingCart
        shopCartState={{ cartOpen, setCartOpen }}
        productState={{ cartProducts, setCartProducts }}
      >
        Cart
      </ShoppingCart>
