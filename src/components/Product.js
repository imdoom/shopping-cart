  ...
  return (
    <Column key={key} size="one-quarter">
      <Notification color="white" textAlign="centered">
        ...
        <Button.Group align="centered" badge={sizeError} badgeColor="danger">
          {Object.keys(inventory).length > 0
            ? Object.keys(inventory[product.sku])
                .filter((s) => inventory[product.sku][s] > 0)
                .map((s) => (
                  <Button
                    className="sizeBtn"
                    onClick={() => {
                    ...
                    color={
                      selectedSize && !resetSize && selectedSize === s
                        ? "black"
                        : ""
                    }
                  >
